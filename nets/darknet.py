...
class SPPBottleneck(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_sizes=(5, 9, 13), activation="silu"):
        ...
        # self.Conv_rep1 = conv_rep(hidden_channels, hidden_channels, 3, 1)
        # self.Conv_rep2 = conv_rep(hidden_channels, hidden_channels, 3, 1)
        ...
        self.conv2      = BaseConv(conv2_channels, hidden_channels, 1, stride=1, act=activation)
        self.conv3      = BaseConv(in_channels, out_channels, 1, stride=1, act=activation)


    def forward(self, x):
        x0 = self.conv1(x)
        # x0 = self.Conv_rep1(x0)
        x1 = self.m1(x0)
        x2 = self.m2(x1)
        x3 = self.m3(x2)
        ...
        y1 = self.conv2(x4)
        # y1 = self.Conv_rep2(y1)
        y2 = self.conv1(x)
        # x5 = torch.cat([y1]+[y2], dim=1)
        x5 = torch.cat((y1, y2), 1)
        x6 = self.conv3(x5)
        return x6
